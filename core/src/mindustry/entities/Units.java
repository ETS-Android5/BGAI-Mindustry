...
public class Units {
    ...
    public static int getCap(Team team) {
        ...
        if ((team == state.rules.waveTeam && !state.rules.pvp)
                || (state.isCampaign() && team == state.rules.waveTeam)) {
            return Integer.MAX_VALUE;
        }
        return Math.max(0,
                state.rules.unitCapVariable ? state.rules.unitCap + team.data().unitCap : state.rules.unitCap);
    }

    /**
     * @return unit cap as a string, substituting the infinity symbol instead of
     *         MAX_VALUE
     */
    ...
    /**
     * @return whether this player can interact with a specific tile. if either of
     *         these are null, returns true.
     */
     ...
     * @param range  The maximum distance from the target X/Y the targeter can be
     *               for it to be valid
    ...
    public static boolean invalidateTarget(Posc target, Team team, float x, float y, float range) {
        return target == null
                || (range != Float.MAX_VALUE
                        && !target.within(x, y, range + (target instanceof Sized hb ? hb.hitSize() / 2f : 0f)))
    ...
    /**
     * Returns the closest target enemy. First, units are checked, then tile
     * entities.
     */
    ...
    public static Teamc closestTargetDangerous(Team team, float x, float y, float range, Boolf<Unit> unitPred,
            Boolf<Building> tilePred) {
        if (team == Team.derelict)
            return null;
     ...
     */
    public static Teamc bestTarget(Team team, float x, float y, float range, Boolf<Unit> unitPred,
            Boolf<Building> tilePred, Sortf sort) {
        if (team == Team.derelict)
            return null;
    ...
    public static Unit closest(Team team, float x, float y, Boolf<Unit> predicate) {
        ...
        for (Unit e : Groups.unit) {
            if (!predicate.get(e) || e.team() != team)
                continue;
     ...
     * Unlike the closest() function, this only guarantees that unit hitboxes
     * overlap the range.
     */
