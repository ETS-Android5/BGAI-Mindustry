...
public class AIController implements UnitController {
    ...
    /**
     * For ground units: Looks at the target, or the movement position. Does not
     * apply to non-omni units.
     */
    ...
    public void pathfind(int pathTarget) {
        ...
        System.out.println("in pathfind-> pathTarget= " + pathTarget);
        System.out.println("in pathfind-> costType= " + costType);
        Tile tile = unit.tileOn();
        if (tile == null)
            return;
        Tile targetTile = pathfinder.getTargetTile(tile, pathfinder.getField(unit.team, costType, pathTarget));
        System.out.println("in pathfind-> targetTile= " + targetTile.x + " " + targetTile.y);
    ...
    public void pathfind(int pathTarget, boolean[] neighborInRange, int[] neighbordDist) {
        int costType = unit.pathType();
        System.out.println("in pathfind-> pathTarget= " + pathTarget);
        System.out.println("in pathfind-> costType= " + costType);
        Tile tile = unit.tileOn();
        if (tile == null)
            return;
        Tile targetTile = pathfinder.getTargetTile(tile, pathfinder.getField(unit.team, costType, pathTarget),
                neighborInRange, neighbordDist);
        System.out.println("in pathfind-> targetTile= " + targetTile.x + " " + targetTile.y);

        if (tile == targetTile || (costType == Pathfinder.costNaval && !targetTile.floor().isLiquid))
            return;

        // while (unit.tileOn() != targetTile) {
        // unit.movePref(vec.trns(unit.angleTo(targetTile.worldx(),
        // targetTile.worldy()), unit.speed()));
        // }
        unit.movePref(vec.trns(unit.angleTo(targetTile.worldx(), targetTile.worldy()), unit.speed()));
        System.out.println("final tile= " + unit.tileOn().x + " " + unit.tileOn().y);
    }
    ...
    public void updateWeapons() {
        ...
        for (var mount : unit.mounts) {
            ...
            if (unit.type.singleTarget) {
                ...
                if (ret) {
                    mount.target = findTarget(mountX, mountY, weapon.bullet.range(), weapon.bullet.collidesAir,
            ...
            if (mount.target != null) {
                shoot = mount.target.within(mountX, mountY,
                        weapon.bullet.range() + (mount.target instanceof Sized s ? s.hitSize() / 2f : 0f))
                        && shouldShoot();
    ...
    public Teamc targetFlag(float x, float y, BlockFlag flag, boolean enemy) {
        if (unit.team == Team.derelict)
            return null;
        Tile target = Geometry.findClosest(x, y,
                enemy ? indexer.getEnemy(unit.team, flag) : indexer.getAllied(unit.team, flag));
    ...
    public Teamc targetDangerous(float x, float y, float range, boolean air, boolean ground) {
        return Units.closestTargetDangerous(unit.team, x, y, range, u -> u.checkTarget(air, ground), t -> ground);
    }
    ...
    public void unloadPayloads() {
        if (unit instanceof Payloadc pay && pay.hasPayload() && target instanceof Building
                && pay.payloads().peek() instanceof UnitPayload) {
