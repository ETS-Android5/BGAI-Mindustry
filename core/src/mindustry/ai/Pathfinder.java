...
public class Pathfinder implements Runnable {
    ...
    public static final Seq<PathCost> costTypes = Seq.with(
            ...
            (team, tile) -> (PathTile.allDeep(tile)
                    || (PathTile.team(tile) == team.id || PathTile.team(tile) == 0) && PathTile.solid(tile))
                            ? impassable
                            : 1 +
    ...
    /**
     * unordered array of path data for iteration only. DO NOT iterate or access
    ...
    private int packTile(Tile tile) {
        ...
        return PathTile.get(
                tile.build == null || !solid || tile.block() instanceof CoreBlock ? 0
                        : Math.min((int) (tile.build.health / 40), 80),
                tid == 0 && tile.build != null && state.rules.coreCapture ? 255 : tid, // use teamid = 255 when core
                                                                                       // capture is enabled to mark out
                                                                                       // derelict structures
                solid,
                tile.floor().isLiquid,
    ...
    public Flowfield getField(Team team, int costType, int fieldType) {
        ...
        System.out.println("getField: " + cache[team.id][costType][fieldType]);
