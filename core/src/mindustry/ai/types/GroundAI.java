...
public class GroundAI extends AIController {
    ...
    public void updateMovement() {
        ...
        if (target(unit.x, unit.y, rangeToDetect + 5, true, true) != core && 1 == 0) {
        ...
        if ((target != null && target == target_core && !unit.within(target, unit.type.range * 0.5f))
        ...
        } else if (core != null && unit.within(core, unit.range() + core.block.size * tilesize / 2f)) {
            target = core;
            ...
            if (move)
                pathfind(Pathfinder.fieldCore, neighborInRange, neighborDist);
        }
